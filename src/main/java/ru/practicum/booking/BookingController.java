...
public class BookingController {
    ...
    @GetMapping
    public List<ResponseBookingDto> getAllUsersBookings(@RequestHeader("X-Sharer-User-Id") int userId, @RequestParam (value = "state", defaultValue = "ALL", required = false) BookingState state) {
        log.info("Получен запрос GET /bookings?state={state}");
        return bookingService.getAllUsersBookings(userId, state);
    }

    @GetMapping("/owner")
    public List<ResponseBookingDto> getAllItemOwnerBookings(@RequestHeader("X-Sharer-User-Id") int ownerId, @RequestParam (value = "state", defaultValue = "ALL", required = false) BookingState state) {
